...
class ComparatorUtils:
    ...
    def loadCustomComparators():
        ...
        """
        qaic_logger.info('Searching for custom comparators....')
                    ...
                    if cl[1].__module__ == file:
                        ...
                        for class_h in class_hier:
                            if class_h.__name__ != 'ABC' and class_h.__name__ != 'object' and \
                                ...
                                qaic_logger.info('Loaded custom comparator : {}'.format(_name))
        return registered_comparators
...
class FileComparator:
    ...
    def compare(cls, op1, op2, cmp, np_dtype, save_dir=None):
        ...
        """
        qaic_logger.debug('FileComparator::compare({},{}), dtype {}'.format(op1, op2, np_dtype))
        ismatch = True
        if cmp.name() == "box":
            # checks the existence of given .raw /.bin files
            for path in op1 + op2 :
                if not os.path.exists(path):
                    qaic_logger.error('Input file : {} does not exist '.format(path))
        ...
        if op2_np.shape != op1_np.shape:
            qaic_logger.debug(
                'Handling shape mismatch between two tensors {}: {} vs {}: {}'.format(op1,
                                                                                      op1_np.shape,
                                                                                      op2,
                                                                                      op2_np.shape))
            try:
                ...
                qaic_logger.error('Shape Mismatch {}:{} vs {}:{}'.format(op1, op1_np.shape,
                                                                         op2, op2_np.shape))
                qaic_logger.exception(e)
                raise ce.FileComparatorException(
                    '******** ERROR: Shape Mismatch------- :{} vs {}'.format(op2_np.shape,
                                                                             op1_np.shape))
...
class NormComparator(Comparators):
    ...
    def compare(self, op1, op2):
        ...
        if math.isnan(match_percent):
            qaic_logger.warning('{} match returned NaN, marked as 0% match'.format(self.name))
...
class TolComparator(Comparators):
    ...
    def compare(self, op1, op2):
        ...
        # finds the percentage match between op1 and op2 beased on given tolerance
...
class AvgComparator(Comparators):
    ...
    def compare(self, op1, op2):
        ...
        # check if match_percent results to NaN
        if math.isnan(match_percent):
            qaic_logger.warning('Average match returned NaN, marked as 0% match')
...
class CosComparator(Comparators):
    ...
    def compare(self, op1, op2):
        ...
        # finds the cosine simiarity between op1 and op2 range is [-1, 1]
...
class StdComparator(Comparators):
    ...
    def compare(self, op1, op2):
        ...
        # check if match_percent results to NaN
        if math.isnan(match_percent):
            qaic_logger.warning('Relative StdDev match returned NaN, marked as 0% match')
...
class BoxComparator(Comparators):
    ...
    def compare(self, op1, op2):
        ...
        this algorithm only works for cases that have coordinate systems start with from topleft as the first coordinate 
        and bottom right as the second corrdinate.
        also assuming all inputs are float32 results.
        """
        match_info = ''
        ...
        s_g_box = np.array([item for _,item in sorted(zip(g_score, g_box), key = lambda x: x[0], reverse = True)])
        ...
        for i in range(l_print):
            qaic_logger.info("Class " + str(s_g_cls[i]) + " has IoU: " + \
    ...
    def intersect(self, gt_box, dt_box):
        ...
        """
        bb = [max(dt_box[0], gt_box[0]), max(dt_box[1], gt_box[1]),
            min(dt_box[2], gt_box[2]), min(dt_box[3], gt_box[3])]
