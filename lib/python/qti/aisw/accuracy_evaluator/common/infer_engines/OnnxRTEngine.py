...
class OnnxInferenceEngine(InferenceEngine):
    """
    OnnxInferenceEngine class takes required inputs supplied by user from commandline options and
    ...
    def execute(self):
        ...
        # execution
        qaic_logger.debug("OnnxInferenceEngine start execution")
        res = {}
        ...
        with open(self.input_path) as f:
            for iter, line in enumerate(f):
                ...
                try:
                    ...
                    qaic_logger.error('ort_session.run failed')
                    qaic_logger.exception(e)
                ...
                if self.output_info:
                    ...
                    for out_name in list(self.output_info.keys()):
                        if out_name.startswith('_'):
                            output_names.append(out_name[1:])
                        else:
                ...
                if self.output_path:
                    ...
                    profile_data, _paths = self.save_outputs_and_profile(output_names, outputs, iter,
        ...
        output_array_map = list(zip(input_names + output_names,
        ...
        if do_profile:
            logging.info('Captured onnx profile')

        inf_time = time.time() - start_time
        qaic_logger.debug("OnnxInferenceEngine execution success")
    ...
    def validate(self):
        ...
        """
        qaic_logger.debug("OnnxInferenceEngine validation")
        # check the existence of model path and its autenticity
        if os.path.exists(self.model_path):
            try:
                onnx.checker.check_model(self.model_path)
            except Exception as e:
                qaic_logger.warning(f'check_model failed for model : {self.model_path} Reason: {e}')
                # supressing the exception, as shape info is populated from the onnxrt-session.
        ...
        qaic_logger.debug("OnnxInferenceEngine validation success")
        return True