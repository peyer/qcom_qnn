...
def execute(command, args=[], cwd='.', shell=False, timeout=1800):
    ...
    :return: int, [], []
    """
    logging.debug("Host Command: {} {}".format(command, args))
    ...
    logging.debug("Result Code (%d): stdout: (%s) stderr: (%s)" % (return_code, output, error))
...
class Adb(object):
    ...
    def shell(self, command, args=[]):
        ...
        logging.debug("Executing on the android device")
        logging.debug(shell_args)
    ...
    def __execute(self, command, args, cwd='.'):
        ...
        (return_code, output, error) = execute(self.__adb_executable, adb_command_args, cwd=cwd, timeout=self.ADB_DEFAULT_TIMEOUT)
        # when the process gets killed, it will return -9 code; Logging this error for debug purpose
        if return_code == -9:
            logging.error("adb command didn't execute within the timeout. Is device in good state?")
        return (return_code, output, error)

    def get_devices(self):
        code, out, err = self.__execute('devices', [])
        if code != 0:
            logging.error("Could not retrieve list of adb devices connected, following error "
                         "occured: {0}".format("\n".join(err)))
            return code, out, err
