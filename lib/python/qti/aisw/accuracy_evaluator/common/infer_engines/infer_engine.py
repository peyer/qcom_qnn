...
class InferenceEngine(ABC):
    """
    InferenceEngine class is an abstract class with implemented by different ML frameworks
    to do inference on a set of inputs.
    """
    def __init__(self, model, inputlistfile, output_path, multithread=False,
                 input_info=None, output_info=None, gen_out_file=None, extra_params=None,
                 binary_path=None):
    ...
    def save_outputs_and_profile(self, output_names, outputs, iter, save_outputs,
                                 do_profile):
        """
        ...
        Returns:
            profile_data : dict mapping ouput names and its corresponding tuple of datatype,
        ...
        for i, name in enumerate(output_names):
            ...
            if do_profile:
                if (not outputs[i].size or outputs[i].dtype == np.bool):
                    profile_data[name] = (outputs[i].dtype,
                                          outputs[i].shape,
                                          outputs[i],
                                          outputs[i],
                                          outputs[i])
