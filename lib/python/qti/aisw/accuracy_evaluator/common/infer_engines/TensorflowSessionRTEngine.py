...
class TensorflowSessionInferenceEngine(InferenceEngine):
    """
    TensorflowInferenceEngine class takes required inputs supplied by user from commandline
    options and
    ...
    def execute(self):
        ...
        """
        qaic_logger.debug("TensorflowInferenceEngine start execution")
        ...
        if self.input_info is None:
            qaic_logger.error("Please pass input-info")
            raise ce.ConfigurationException("input-info is empty")
        if self.output_info is None:
            qaic_logger.error("Please pass output-info")
        ...
        try:
            output_tensors = [self.sess.graph.get_tensor_by_name(out_name+':0') for out_name in output_names]
        except Exception as e:
            qaic_logger.error('Creating output tensors for inference failed.')
        ...
        with open(self.input_path) as f:
            for iter, line in enumerate(f):
                ...
                for idx, inp in enumerate(inps):
                    ...
                    input_dict[self.sess.graph.get_tensor_by_name(inp_nodes[idx].name+':0')] = input_np
                ...
                if self.output_path:
                    ...
                    profile_data, _paths = self.save_outputs_and_profile([ModelHelper._replace_special_chars(name) for name in output_names], outputs,
                                                                         iter,
                                                                         True,
                                                                         do_profile)
        ...
        if do_profile:
            logging.info('Captured tf profile')

        qaic_logger.debug("TensorflowInferenceEngine execution success")
    ...
    def validate(self):
        ...
        """
        qaic_logger.debug("TensorflowInferenceEngine validation")
        ...
        if not isinstance(self.model_path, tf.compat.v1.Session):
            qaic_logger.error('Model passed is not an instance of tensorflow.compat.v1.Session')
            raise ce.InferenceEngineException('Model passed is not an instance of tensorflow.compat.v1.Session')
        ...
        qaic_logger.debug("TensorflowInferenceEngine validation success")
        return True
