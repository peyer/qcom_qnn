...
class TensorflowInferenceEngine(InferenceEngine):
    ...
    def execute(self):
        ...
        if '-save-single-layer-output-name' in self.extra_params:
            for i, node in enumerate(graph_def.node):
                ...
                if node.op != "Placeholder" and (
                        self.extra_params['-save-single-layer-output-name'] in node.name):
        ...
        frozen_func = self.wrap_frozen_graph(graph_def=graph_def,
                                             inputs=[node.name + ':0' for node in inp_nodes],
                                             outputs=[out_name + ':0' for out_name in
                                                      out_node_names],
                                             )
                    ...
                    if self.input_info is None:
                        ...
                        try:
                            input_np = np.fromfile(inp,
                                                   dtype=Helper.tf_type_to_numpy(
                                                       inp_nodes[idx].attr['dtype'].type)).reshape(
                                inp_shapes_map[inp_nodes[idx].name])
                            input_list.append(tf.convert_to_tensor(input_np,
                                                                   Helper.tf_type_to_numpy(
                                                                       inp_nodes[idx].attr[
                                                                           'dtype'].type)))
                        ...
                        input_list.append(tf.convert_to_tensor(input_np, Helper.get_np_dtype(
                            self.input_info[inp_nodes[idx].name] \
                                [0])))
