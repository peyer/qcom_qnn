...
class TorchScriptInferenceEngine(InferenceEngine):
    """
    TorchScriptInferenceEngine class takes required inputs supplied by user from commandline options and
    ...
    def execute(self):
        ...
        # execution
        qaic_logger.debug("TorchScriptInferenceEngine start execution")
        res = {}
        ...
        # running the session
        inp_nodes = [ip.debugName().split('.')[0] for ip in list(loaded_model.graph.inputs())
        ...
        with open(self.input_path) as f:
            for iter, line in enumerate(f):
                ...
                try:
                    ...
                    qaic_logger.error('TorchScript run failed')
                    qaic_logger.exception(e)
        ...
        if do_profile:
            logging.info('Captured TorchScript profile')

        inf_time = time.time() - start_time
        qaic_logger.debug("TorchScriptInferenceEngine execution success")
    ...
    def validate(self):
        ...
        qaic_logger.debug("TorchScriptInferenceEngine validation success")
        return True