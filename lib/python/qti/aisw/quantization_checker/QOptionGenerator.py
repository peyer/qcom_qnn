...
class QOptionGenerator:
    def __init__(self, quantizationOptions, quantizationAlgorithms, modelFile, inputList, sdkDir, sdkType, activationWidth, biasWidth, weightWidth, outputDir, quantOverridesPath, configFile, logger):
    ...
    def generate(self):
        ...
        try:
            if converter != None:
                ...
                resultsMap, quantizationVariationsWithCommand = converter.convert(env=environment, activationWidth=self.activationWidth, biasWidth=self.biasWidth, weightWidth=self.weightWidth, outputDir=self.outputDir, quantOverrides=self.quantOverridesPath)
