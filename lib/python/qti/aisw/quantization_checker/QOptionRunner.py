...
class QOptionRunner:
    ...
    def __buildModel(self):
        ...
        modelLibGeneratorCmd = os.path.join(self.sdkDir, Constants.BIN_PATH_IN_SDK, Constants.QNN_MODEL_LIB_GENERATOR_BIN_NAME)
        modelLibGeneratorArgs = ' -c ' + os.path.join(outputPathName, self.quantizationOption + '.cpp') + ' -b ' + os.path.join(outputPathName, self.quantizationOption + '.bin') + ' -l lib' + self.quantizationOption + '.so -o ' + self.outputDir + ' -t ' + Constants.X86_LINUX_CLANG
        modelLibGeneratorCmdAndArgs = modelLibGeneratorCmd + modelLibGeneratorArgs
        self.logger.print('Building model...')
        result = utils.issueCommandAndWait(modelLibGeneratorCmdAndArgs, self.logger, em.getEnvironment(self.configParams, self.sdkDir, Constants.QNN), False, True)
        if result != 0:
