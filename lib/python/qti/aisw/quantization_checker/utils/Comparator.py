        ...
        for opName in opsMap[quantizationVariation]:
            ...
            for unquantOpName in unquantizedOps:
                ...
                if quantizedOp.getActivationNodeName() not in (None, '') and quantizedOp.getActivationNodeName() == unquantizedOp.getActivationNodeName():
                    ...
                    perInputResults = ResultSet()
                    for activationPerInput in activationList:
                        # analyze the unquantized activations but do not compare them to anything
                        if quantizationVariation == Constants.UNQUANTIZED:
                            perInputResults.add(activationPerInput[0], ActivationResultNode(unquantOpName, unquantizedOp.getActivationNodeName(), comparePerOpActivations(activationPerInput[1], None, None, comparisonAlgorithms), None, None))
                        else:
                            perInputResults.add(activationPerInput[0], ActivationResultNode(opName, quantizedOp.getActivationNodeName(), comparePerOpActivations(activationPerInput[1], quantizedOp.getActivationScale(), quantizedOp.getActivationOffset(), comparisonAlgorithms), quantizedOp.getActivationScale(), quantizedOp.getActivationOffset()))
                    perOpResults.append(perInputResults)
                    break
        results.add(quantizationVariation, perOpResults)
    return results
