...
class TensorflowConverter(Converter):
    ...
    def convert(self, env, activationWidth=None, biasWidth=None, weightWidth=None, outputDir=None, quantOverrides=None):
    ...
    def __convertQnn(self, baseArgs, activationWidth, biasWidth, weightWidth, quantOverrides, outputDir, tfConverterBinaryName, env):
        ...
        tensorflowConverterBinaryPath = os.path.join(self.sdkPath, Constants.BIN_PATH_IN_SDK, tfConverterBinaryName)
    ...
    def __convertSnpe(self, baseArgs, activationWidth, biasWidth, weightWidth, quantOverrides, outputDir, tfConverterBinaryName, env):
        ...
        resultsMap = {}
        quantizationVariationsWithCommand = {}
        snpeDlcQuantizeBinaryPath = os.path.join(self.sdkPath, Constants.BIN_PATH_IN_SDK, Constants.SNPE_QUANTIZER_BIN_NAME)
        for quantizationVariation, params in self.__tfConverterArgs.items():
            self.fileHelper.makeSubdir(os.path.join(outputDir, quantizationVariation))
...
class TfliteConverter(Converter):
    ...
    def convert(self, env, activationWidth=None, biasWidth=None, weightWidth=None, outputDir=None, quantOverrides=None):
        ...
        with ScopedFileUtils(workingDir, self.fileHelper):
            ...
            if self.sdkType.upper() == 'SNPE':
                resultsMap, quantizationVariationsWithCommand = self.__convertSnpe(baseArgs, activationWidth, biasWidth, weightWidth, quantOverrides, workingDir, Constants.SNPE_TFLITE_CONVERTER_BIN_NAME, env)
            else:
                resultsMap, quantizationVariationsWithCommand = self.__convertQnn(baseArgs, activationWidth, biasWidth, weightWidth, quantOverrides, workingDir, Constants.QNN_TFLITE_CONVERTER_BIN_NAME, env)
    ...
    def __convertQnn(self, baseArgs, activationWidth, biasWidth, weightWidth, quantOverrides, workingDir, tfliteConverterBinaryName, env):
        ...
        resultsMap = {}
        quantizationVariationsWithCommand = {}
        tfliteConverterBinaryPath = os.path.join(self.sdkPath, Constants.BIN_PATH_IN_SDK, tfliteConverterBinaryName)
...
class OnnxConverter(Converter):
    ...
    def convert(self, env,  activationWidth=None, biasWidth=None, weightWidth=None, outputDir=None, quantOverrides=None):
        ...
        with ScopedFileUtils(workingDir, self.fileHelper):
            ...
            resultsMap = {}
            if self.sdkType.upper() == 'SNPE':
                resultsMap, quantizationVariationsWithCommand = self.__convertSnpe(baseArgs, activationWidth, biasWidth, weightWidth, quantOverrides, workingDir, Constants.SNPE_ONNX_CONVERTER_BIN_NAME, env)
            else:
                resultsMap, quantizationVariationsWithCommand = self.__convertQnn(baseArgs, activationWidth, biasWidth, weightWidth, quantOverrides, workingDir, Constants.QNN_ONNX_CONVERTER_BIN_NAME, env)
    ...
    def __convertQnn(self, baseArgs, activationWidth, biasWidth, weightWidth, quantOverrides, workingDir, onnxConverterBinaryName, env):
        ...
        resultsMap = {}
        quantizationVariationsWithCommand = {}
        onnxConverterBinaryPath = os.path.join(self.sdkPath, Constants.BIN_PATH_IN_SDK, onnxConverterBinaryName)
