...
class OptimizeGruTranslation(OptimizationTranslationBase):
    ...
    def unroll_gru_time_steps(self, graph):
        ...
        for nodes_tuple in matched_node_list:
            ...
            for i in range(seq_length):
                ...
                gru_time_step_i_op_name = gru_node_name + '_' + str(i)
    ...
    def expand_gru_update_gate(self, graph, gru_node, input_x_reshape_node_name, input_h_reshape_node_name):
        ...
        parent_gru_node_name = gru_node_name.rpartition('_')[0] if gru_node_name.rpartition('_')[0] else gru_node_name
        input_to_control_w_name = parent_gru_node_name + '_input_to_control_w'
        if not graph.has_buffer(input_to_control_w_name):
        ...
        recurrent_to_control_w_name = parent_gru_node_name + '_recurrent_to_control_w'
        if not graph.has_buffer(recurrent_to_control_w_name):
        ...
        control_gate_b_name = parent_gru_node_name + '_control_gate_b'
        if not graph.has_buffer(control_gate_b_name):
    ...
    def expand_gru_reset_gate(self, graph, gru_node, input_x_reshape_node_name, input_h_reshape_node_name):
        ...
        parent_gru_node_name = gru_node_name.rpartition('_')[0] if gru_node_name.rpartition('_')[0] else gru_node_name
        input_to_forget_w_name = parent_gru_node_name + '_input_to_forget_w'
        if not graph.has_buffer(input_to_forget_w_name):
        ...
        recurrent_to_forget_w_name = parent_gru_node_name + '_recurrent_to_forget_w'
        if not graph.has_buffer(recurrent_to_forget_w_name):
        ...
        forget_gate_b_name = parent_gru_node_name + '_forget_gate_b'
        if not graph.has_buffer(forget_gate_b_name):
    ...
    def expand_gru_hidden_gate(self, graph, gru_node, input_x_reshape_node_name, input_h_reshape_node_name, rt, linear_before_reset = 0):
        ...
        parent_gru_node_name = gru_node_name.rpartition('_')[0] if gru_node_name.rpartition('_')[0] else gru_node_name
        input_to_state_w_name = parent_gru_node_name + '_input_to_state_w'
        if not graph.has_buffer(input_to_state_w_name):
        ...
        recurrent_to_state_w_name = parent_gru_node_name + '_recurrent_to_state_w'
        if not graph.has_buffer(recurrent_to_state_w_name):
        ...
        state_gate_fwd_b_name = parent_gru_node_name + '_state_gate_fwd_b'
        if not graph.has_buffer(state_gate_fwd_b_name):
        ...
        state_gate_rec_b_name = parent_gru_node_name + '_state_gate_rec_b'
        if not graph.has_buffer(state_gate_rec_b_name):
        ...
        if linear_before_reset == 0:
            ...
            ht_1_rh_rbh_matmul_op_name = gru_node_name + "_ht_1_rh_rbh_matmul_op"
            Rbh = np.zeros(graph.get_buffer(state_gate_rec_b_name).shape[-1], dtype=np.float32)
    ...
    def update_gru_hidden_state(self, graph, gru_node, Ht_1, zt, ht):
        ...
        parent_gru_node_name = gru_node_name.rpartition('_')[0] if gru_node_name.rpartition('_')[0] else gru_node_name
        initial_ones_op_name = parent_gru_node_name + '_initial_ones_op'
...
class OptimizeLayerNormTranslation(OptimizationTranslationBase):
    ...
    def match_layer_norm(graph):
        ...
        for sequence in sequences:
            matched_node_list = graph.get_matched_nodes(sequence, ignore_constants=True)
            is_first_node_reduce = sequence[0][0] == "reduce_mean"

            for node_tuple in matched_node_list:
                if is_first_node_reduce:
