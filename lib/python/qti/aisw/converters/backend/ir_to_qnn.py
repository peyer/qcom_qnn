...
try:
    ...
    if sys.version_info[0] == 3 and sys.version_info[1] == 6:
        from qti.aisw.converters.common import libPyIrJsonSerializer36 as json_serializer
    else:
        from qti.aisw.converters.common import libPyIrJsonSerializer as json_serializer
...
class QnnConverterBackend(QnnConverterBackendBase):
    ...
    def add_tensor_to_qnn_bin(self, tensor_name, tensor):
        ...
        if tensor.dtype == np.float32 and self.float_bw == 16:
            tensor = self.c_utils.buffer_fp32_to_fp16(tensor.flatten())
    ...
    def save(self, graph):
        ...
        if self.quantizer.opts.input_list:
            self.is_online_construction = True
        ir_graph = self.get_ir_graph(graph)
        self.is_online_construction = False

        user_custom_io = False
        if graph.user_custom_io:
            user_custom_io = True
        if graph.preserve_io_datatype_passed:
            self.quantizer.opts.use_native_input_dtype = True
        self.quantizer.quantize(ir_graph, self, user_custom_io)

        if graph.preserve_io_datatype_passed:
            ir_graph.preserve_datatype(graph.preserve_datatype_tensors)
