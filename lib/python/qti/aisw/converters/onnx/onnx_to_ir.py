...
class OnnxConverterFrontend(ConverterFrontend):
    ...
    def add_composable_op(self, src_op, model):
        ...
        for elem_op in expanded_nodes:
            # first check whether the op is a custom op or not
            if self.custom_op_factory and elem_op.op_type in [operator.type_name for operator in self.custom_op_factory.custom_opdefs]:
    ...
    def convert(self):
        ...
        # the onnx simplifier.
        self.populate_composable_custom_op_collection(model)
        ...
        for i, src_op in enumerate(model.graph.node):
            ...
            try:
                # first check whether the op is a composable op or not
                # If so, then expand the composable operation and add individual nodes in the expansion.
                if self.composable_custom_op_collection and self.composable_custom_op_collection.is_composable_op(src_op):
                    self.add_composable_op(src_op, model)

                # check if layer is a registered custom op in an op collection.
                # If so, the layer is added and the outer loop continues.
                elif self.custom_op_factory and src_op.op_type in self.custom_op_factory.op_collection:
