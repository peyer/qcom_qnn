...
* e.g. DEF_PACKAGE_OP((${operator.type_name.lower()}Impl${_get_template_tensors()}), "${operator.type_name}")
*/
DEF_PACKAGE_OP((${operator.type_name.lower()}Impl${_get_template_tensors()}), "${operator.type_name}")
...
* e.g. DEF_PACKAGE_OP_AND_COST_AND_FLAGS((${operator.type_name.lower()}Impl${_get_template_tensors("PlainFloatTensor")}), "${operator.type_name}", SNAIL)
*/
...
* e.g. DEF_PACKAGE_OP_AND_COST_F_AND_FLAGS((${operator.type_name.lower()}Impl${_get_template_tensors("PlainFloatTensor")}),
...
<%def name="_get_template_tensors(tensor_type=None)" filter="n" buffered="True" >
<%
num_tensors = len(_template_builder(operator).split(','))
tensor_type = "Tensor" if tensor_type is None else str(tensor_type)
%>
${"<" + ', '.join([tensor_type]*num_tensors) + ">"}
</%def>
