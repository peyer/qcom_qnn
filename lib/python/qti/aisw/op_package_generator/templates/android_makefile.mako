...
# setup include paths
PACKAGE_C_INCLUDES += -I $(QNN_SDK_ROOT)/include/QNN -I $(QNN_SDK_ROOT)/include/QNN/${package_info.backend.upper()} -I $(LOCAL_PATH)/../include/ -I $(UTIL_SRC_DIR) -I $(UTIL_SRC_DIR)/${package_info.backend.upper()} -I $(CUSTOM_OP_DIR)
# copy source files from SDK if not present
$(info Copying custom op source files from SDK)
COPYFILES := $(shell find $(CUSTOM_OP_DIR)/${package_info.backend.upper()} -name "*.cpp" -exec cp -rf {} $(LOCAL_PATH)/../src 2>/dev/null \;)
else
...
LOCAL_MODULE                   := ${package_info.name}
