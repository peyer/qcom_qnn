...
class QnnOpPackageGenerator:
    ...
    def parse_config(self, config_paths: List[str], output_path: Optional[str] = None):
        """
    ...
    def setup_file_paths(self, force_generation=False, gen_cmakelists=False):
        ...
        for i, package_info in enumerate(self.package_infos):
            ...
            if package_info.backend == "HTP" or package_info.backend == "HTPMCP" or package_info.backend == "HTP_FP16" or \
                    package_info.backend == "CPU" or package_info.backend == "DSP" or package_info.backend == "GPU":
...
class QnnOpPackageCodeGenerator:
    ...
    def __init__(self, handler: Optional[IOHandler] = None):
        ...
        self.operator_to_resource_name = {}
    ...
    def register_operator_file_handler_with_template(self, file_handler_name, template,
        ...
        self.operator_to_resource_name[operator_name] = file_handler_name
    ...
    def substitute_templates(self, package_info: QnnPackageInfo,
        ...
        for file_handler_name, template in self.handler_to_template_map.items():
            if file_handler_name in self.operator_to_resource_name.values():
                operator_name = [name for name, r_name in self.operator_to_resource_name.items()
                                 if r_name == file_handler_name][0]
        ...
        if file_to_template_map is not None:
            for resource_name, template_path in file_to_template_map.items():
                ...
                writable_content_map[resource_name] = self.__substitute_template(package_info,
                                                                                 self.reader.
    ...
    def __substitute_template(self, package_info: QnnPackageInfo,
        ...
        writable_content = self.render_templates(package_info, template_type,
                                                 template_path=template_path,
                                                 **per_operator_args)
