...
Udo_ErrorType_t relu_executeOp(QnnOpPackage_GlobalInfrastructure_t globalInfra,
  ...
  OpParams_t *operationParams = (OpParams_t *)operation;
  const char *opType          = ((ReluOpFactory_t *)(operationParams->opFactory))->opType;
  if (opType == NULL) {
    return UDO_INVALID_ARGUMENT;
  }
  if (strcmp(opType, g_reluOpType) == 0) {
     ...
     * add code here
     */
    Udo_TensorParam_t *in_tensor  = &(operationParams->InputParams[0]);
    Udo_TensorParam_t *out_tensor = &(operationParams->outputParams[0]);

    if (in_tensor->layout == SNPE_UDO_LAYOUT_NULL || out_tensor->layout == SNPE_UDO_LAYOUT_NULL) {
    ...
    out_tensor->dataType = SNPE_UDO_DATATYPE_FIXED_8;
    if ((*(globalInfra->dspGlobalInfra->hexNNv2Infra.udoSetOutputTensorSize))(
            operationParams->opInfra, 0, inputLen) != 0) {
      return SNPE_UDO_UNSUPPORTED_FEATURE;
    }
    ...
    (*(globalInfra->dspGlobalInfra->hexNNv2Infra.udoRunWorkerThreads))(
        operationParams->opInfra, 1, workerThreadReluQuant, &workerThreadIn);
    return UDO_NO_ERROR;
  } else {
    return UDO_INVALID_ARGUMENT;
  }
