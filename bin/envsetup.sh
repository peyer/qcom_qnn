...
function _setup_aisw_sdk()
{
  ...
  export PYTHONPATH="${AISW_SDK_ROOT}/benchmarks/QNN/":"${AISW_SDK_ROOT}/lib/python/":${PYTHONPATH}
  ...
  if ls ${AISW_SDK_ROOT}/bin/x86_64-linux-clang/hexagon-* > /dev/null 2>&1; then
    export HEXAGON_TOOLS_DIR=${AISW_SDK_ROOT}/bin/x86_64-linux-clang
  fi
}
