...
} __attribute__((packed)); // <- so the struct is not considered aligned
...
inline void __attribute__((always_inline)) l2pref(const void *p, uint32_t height, uint32_t width, uint32_t stride)
{
    uint64_t const control = Q6_P_combine_RR(stride, Q6_R_combine_RlRl(width, height));
    asm volatile(" l2fetch(%0,%1) " : : "r"(p), "r"(control));
}

inline void __attribute__((always_inline)) pause_just_enough()
{
...
static inline __attribute__((unused, always_inline)) HVX_Vector Q6_Vqf32_equals_Vsf(HVX_Vector vin)
{
    HVX_Vector vzero = Q6_V_vsplat_R(0);
    return Q6_Vqf32_vadd_VsfVsf(vin, vzero);
}

static inline __attribute__((unused, always_inline)) HVX_Vector Q6_Vqf32_from_int(HVX_Vector vin)
{
...
inline void __attribute__((always_inline)) scatter_release_and_stall(const void *p) // must point to TCM
{
...
inline void __attribute__((unused, always_inline)) scatter_release_and_stall(const void *p)
{
    check_hvx();
