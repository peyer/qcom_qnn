...
class MatchOpBase {
  protected:
    ...
    MatchOpBase(MatchAstNode const *, int matchcount, std::vector<std::pair<operand_tag_t, int>> &&operindex);
  ...
  public:
    ...
    std::map<OpId, operand_tag_parm_t> get_inverse_map(MatchOpState const &m) const;
...
class Replacement : public Constraint {
  ...
  private:
    ...
    OpRef do_ResizeDim(OpDef const &old, int dim, int size, ReplFunc const &f, bool reduce_dim = false,
    ...
    static ReplFunc WITH_output_like(ReplFunc_or_Operand &&ref, ReplFunc &&f, int mode);

    // implements WrapOp and WrapOpAlways
    static ReplFunc WrapOp_internal(char const *op_name, char const *package, ReplFunc_or_Operand &&in_op,
                                    bool is_idem); // True for WrapOp, false for WrapOpAlways

    // implements WrapOp("op", "parmname") specifically
    static ReplFunc WrapOp_quick(char const *op_name, char const *package, char const *parm);
  ...
  private:
    ...
    inline static ReplFunc __attribute__((visibility("hidden"))) WrapOp(char const *opname, ReplFunc_or_Operand &&f)
    {
        return WrapOp_internal(opname, pkg_flag.c_str(), std::move(f), true);
    }
    inline static ReplFunc __attribute__((visibility("hidden"))) WrapOp(char const *opname, char const *operand)
    {
        return WrapOp_quick(opname, pkg_flag.c_str(), operand);
    }
    inline static ReplFunc __attribute__((visibility("hidden")))
    WrapOpAlways(char const *opname, ReplFunc_or_Operand &&f)
    {
    ...
    static ReplFunc WITH_OUTPUT_TYPE(DType dtype, int32_t zero_offset, float stepsize, ReplFunc_general &&f);

    // same thing, where the inputs are all function objects...

    static ReplFunc WITH_OUTPUT_TYPE_func(ReplFuncDType &&dtype, ReplFuncInt &&zero_offset, ReplFuncFloat &&stepsize,
                                          ReplFunc &&f);
    ...
    OpRef __attribute__((visibility("hidden"))) immed_SHAPEFN_APPLY(const OpDef &old, F_T f, Arg_Ts &&...args)
    {
    ...
    template <typename F_T, typename... Arg_Ts>
    static ReplFunc __attribute__((visibility("hidden"))) SHAPEFN_APPLY(F_T f, Arg_Ts... args)
    {
        return ReplFunc::create([=](Replacement &rpx, const OpDef &old) -> OpRef {
            /* Call f(rpx,args...) */
    ...
    AUTOSPLIT_SHAPEFN_APPLY(F_T f, hnnx::split_context_tag_t whatsplit, Arg_Ts... args)
    {
        return ReplFunc::create([=](Replacement &rpx, const OpDef &old) -> OpRef {
    ...
    ///
    static ReplFunc AUTOSPLIT_SLICE(ReplFunc_or_Operand &&in, ReplFunc_or_Operand &&start, ReplFunc_or_Operand &&size);
    ...
    static ReplFunc TYPICAL_SLICE(ReplFunc_or_Operand &&in, hnnx::split_context_tag_t whatsplit);
    ...
    static ReplFunc CHANGEDIM_SLICE(ReplFunc_or_Operand &&in, hnnx::split_context_tag_t whatsplit, int newdim);
    ...
    static ReplFunc CHANGEDIM_SLICE(char const *in_parm, char const *whatsplit, int newdim);
    ...
    OpRef do_TYPICAL_SLICE(OpDef const &old, OpRef input_op, hnnx::split_context_tag_t whatsplit, int newdim,
                           bool reduce_dim = false);

    OpRef do_AUTOSPLIT(OpDef const &old, int dim, Split_Context &splitinfo, int chunksize, ReplFunc const &f,
                       bool reduce_dim = false, bool autothread = false);

    OpRef do_AUTOTHREAD(OpDef const &old, int dim, hnnx::split_context_tag_t varname, int ntiles, ReplFunc const &f);
    ...
    static ReplFunc AUTOSPLIT(int dim, hnnx::split_context_tag_t varname, int chunksize, ReplFunc_general &&f);

    static ReplFunc AUTOSPLIT_func(ReplFuncInt &&dim, hnnx::split_context_tag_t varname, ReplFuncInt &&chunksize,
                                   ReplFunc &&f);

    template <oExp::Variant V1, typename T1, oExp::Variant V2, typename T2>
    inline static ReplFunc __attribute__((visibility("hidden")))
    AUTOSPLIT(oExp::expr<V1, T1> &&dim, hnnx::split_context_tag_t varname, oExp::expr<V2, T2> &&chunksize,
              ReplFunc_general &&f)
    {
    ...
    inline static ReplFunc __attribute__((visibility("hidden")))
    AUTOSPLIT(int dim, hnnx::split_context_tag_t varname, oExp::expr<V2, T2> &&chunksize, ReplFunc_general &&f)
    {
    ...
    inline static ReplFunc __attribute__((visibility("hidden")))
    AUTOSPLIT(oExp::expr<V1, T1> &&dim, hnnx::split_context_tag_t varname, int chunksize, ReplFunc_general &&f)
    {
        return AUTOSPLIT_func(oExp::wrap_as_function<int>(std::move(dim)), varname,
    ...
    static ReplFunc AUTOTHREAD_HVX(int dim, hnnx::split_context_tag_t varname, ReplFunc_general &&f);
    // Same for options.autothread_hmx_ntiles.
    static ReplFunc AUTOTHREAD_HMX(int dim, hnnx::split_context_tag_t varname, ReplFunc_general &&f);

    /// AUTOSPLIT and reduce the dim
    static ReplFunc AUTOSPLIT_REDUCE(int dim, hnnx::split_context_tag_t varname, ReplFunc_general &&f);

    static ReplFunc TYPICAL_SLICE_REDUCE(ReplFunc_or_Operand &&in, hnnx::split_context_tag_t whatsplit);
    ...
    ///
    static ReplFunc OP_ITER(ReplFunc &&op_base, hnnx::split_context_tag_t varname, int lo_index, int hi_index,
                            ReplFunc_general &&f);

    // same with ReplFuncInt for the index values, so they can be expressions
    static ReplFunc OP_ITER_func(ReplFunc &&op_base, hnnx::split_context_tag_t const &varname, ReplFuncInt &&lo_index,
                                 ReplFuncInt &&hi_index, ReplFunc &&f);

    // template to map expressions to ReplFuncInt
    template <typename TLO, typename THI>
    inline static ReplFunc __attribute__((visibility("hidden")))
    OP_ITER(ReplFunc &&op_base, hnnx::split_context_tag_t varname, TLO &&lo_index, THI &&hi_index, ReplFunc_general &&f)
    {
    ...
    OpRef do_OP_ITER(OpDef const &old, OpDef const &base_op, Split_Context &splitinfo, int lo_index, int hi_index,
                     ReplFunc const &f);
    ...
    ///
    static ReplFunc __attribute__((visibility("hidden"))) EXTERNAL_REPLACE(external_replace_funcp f)
    {
    ...
    static ReplFunc Op_inner(char const *str, char const *package, int n_in, ReplFunc const *ifuncs);
    ...
    template <typename... Ts> static ReplFunc __attribute__((visibility("hidden"))) Op(char const *str, Ts... ts)
    {
    ...
    static ReplFunc SELECT(bool sel, ReplFunc_general &&iftrue, ReplFunc_general &&iffalse);
    static ReplFunc SELECT(bool sel, ReplFunc_or_Operand &&iftrue, ReplFunc_or_Operand &&iffalse);
    ...
    // of the functions).
    static ReplFunc SELECT_func(ReplFuncBool &&sel, ReplFunc_or_Operand &&iftrue, ReplFunc_or_Operand &&iffalse);
  ...
  public:
    ...
    static OpRef gen_node(const hnnx::opname_tag_t str, std::vector<OpRef> const &inputs, const OpDef &old,
                          char const *package_name = THIS_PKG_NAME_STR, const OpDef *model = nullptr);
    static OpRef gen_node(const hnnx::opname_tag_t str, size_t n_in, OpRef const *inputs, const OpDef &old,
                          char const *package_name = THIS_PKG_NAME_STR, const OpDef *model = nullptr);
    ...
    OpRef gen_Const_int32_common(const OpDef &old, const OutputDef &out_def, const uint8_t *data, size_t data_len);

    OpRef gen_Const_float_common(const OpDef &old, const OutputDef &out_def, const uint8_t *data, size_t data_len);
...
class GraphOptInfo {
  ...
  public:
    GraphOptInfo(int priority, OptimFlags::flags_t flags_in, get_entire_defopt_t defopt_in);

    // This fills in the optimization map.
    static void insert_optimization(std::map<int, GraphOptPass> &opt_passes, GraphOptInfo *p);
    static void populate_package_optimization_map(std::vector<std::unique_ptr<GraphOptInfo>> &opts);
...
class GraphOptPass {
    ...
    class MatchIterator {
        ...
        void advance(); // advance to the next rule and update state
        void advance_select(); // advance to the next rule by testing an input operand
  ...
  public:
    ...
    static MatcherState empty_matcher;
...
#define INIT_PACKAGE_OPTIMIZATION_DEF()                                                                                \
    __attribute__((visibility("hidden"))) std::vector<std::unique_ptr<hnnx::GraphOptInfo>>                             \
