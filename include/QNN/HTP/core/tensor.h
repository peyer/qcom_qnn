...
class Interface {
  protected:
    ...
    static constexpr qparms null_parms = {0, 1.0f, 1.0f};
  ...
  public:
    ...
    virtual void write_floats(void *ptr, const float *srcp, size_t n) const noexcept;
    virtual void read_floats(float *dstp, const void *ptr, size_t n) const noexcept;
    ...
    bool compare_equal(Interface const &) const noexcept; // implements operator == in general case
...
template <> struct hnnx::make_interface<NullInterface> {
    static NullInterface null_ifc; // in tensor.cc
...
template <> void PlainInterface<float>::write_floats(void *ptr, const float *srcp, size_t n) const noexcept;
template <> void PlainInterface<float>::read_floats(float *dstp, const void *ptr, size_t n) const noexcept;

// make_interface for PlainInterface<T>; easy, just have one
// and return a pointer to it.

template <typename T> struct hnnx::make_interface<PlainInterface<T>> {
    static PlainInterface<T> const &get_plain_ifc();
...
template <typename T> struct hnnx::make_interface<ScaleOffsetInterface<T>> {
    ...
    static ScaleOffsetInterface<T> const *from_exemplar(Graph &, ScaleOffsetInterface<T> const &exemplar);
  ...
  protected:
    ...
    static ScaleOffsetInterface<T> const *to_crate(Graph &, ScaleOffsetInterface<T> const &exemplar);
};
...
// This compile-time check determines if two tensor implementation types
// have the same memory layout, i.e. you can static-cast one to the other
// and expect the addressing calculations to work. So, the only diffs which are allowed
// are changing element type to another of the same size, e.g. int8 <-> uint8; and changing memclass.
// Note that this is *not* true for e.g. Int32Tensor vs QuantInt32Tensor, since the interface_type are
// different sizes.
//
//  This is used as a static check in the non-virtual method cast_compatible_layout<TENSB>() of the
//  implementation classes:
//
//  if tensors_compatible_layout<TENSA,TENSB>() is true, it is possible to do
//    TENSA * tensora = ...
//    TENSB * tensorb =   tensora->cast_compatible_layout<TENSB>();
// .. and then access the data via tensorb (as long as you don't use the interface to convert to/from float; those
//    will give the incorrect result if the element type is different. 'raw' access will select the correct element,
//    but may present as the wrong data type (of correct size).
// This is intended for use by the 'tile' access method, and similar, to allow the same code to be used on multiple
// tensor implementation types which are compatible.
//
//
// a 'true' result requires:
//   (1) must have identical layout_type (so, rank is the same, and is the same implementation category
//        (Contig. vs. Indirect)
//   (2) must have identical pad_type
//   (3) element_type can be different, but must be the same size
//   (4) interface_type must be the same, after accounting for possibly different element size
//
template <typename TENSA, typename TENSB> static constexpr bool tensors_compatible_layout()
{
    using traitsA = tensor_traits<TENSA>;
    using traitsB = tensor_traits<TENSB>;
    // it is an error to invoke this where TENSA, TENSB are not implementation types
    static_assert(std::is_same<TENSA, typename traitsA::implementation_type>::value);
    static_assert(std::is_same<TENSB, typename traitsB::implementation_type>::value);

    // must be the same layout_type and pad_type (and therefore the same rank)
    if constexpr (std::is_same<typename traitsA::layout_type, typename traitsB::layout_type>::value &&
                  std::is_same<typename traitsA::pad_type, typename traitsB::pad_type>::value) {
        // element type must be the same size
        using eltype_A = typename traitsA::element_type;
        if constexpr (sizeof(eltype_A) == sizeof(typename traitsB::element_type)) {
            // the A interface must be the same as the B (if we adapt B to A::element_type)
            using adapted_B_interface = typename traitsB::interface_type::template interface_other_type<eltype_A>;
            if constexpr (std::is_same<typename traitsA::interface_type, adapted_B_interface>::value) {
                static_assert(sizeof(TENSA) == sizeof(TENSB)); // make sure we didn't miss anything
                return true;
            }
        }
    }
...
// This is like tensors_compatible_layout, but you can supply a Concrete tensor, or any other
// class which has an implementation_type in its traits; the implementation_type are compared.
// This does *not* prove that you can static_cast a TENSA to a TENSB pointer.
template <typename TENSA, typename TENSB> static constexpr bool tensors_compatible_implementation()
{
    return tensors_compatible_layout<typename tensor_traits<TENSA>::implementation_type,
                                     typename tensor_traits<TENSA>::implementation_type>();
}
...
struct DTypeScaleOff {
    ...
    DTypeScaleOff(DType dt, Interface const &) noexcept;
...
class Tensor {
  public:
    ...
    virtual uint32_t find_content_hash() const noexcept; // find 'content hash' of the data.
  ...
  public:
    ...
    inline void const *element_ptr(size_t rank, const SIdx coords[]) const ALWAYS_INLINE
    {
        return (void const *)element_addr(rank, coords);
    }
    inline void *element_ptr(size_t rank, const SIdx coords[]) ALWAYS_INLINE { return element_addr(rank, coords); }
    ...
    template <typename... ind_types> inline GenericAccessorRO operator()(ind_types... inds) const
    {
        ...
        return this->generic_accessor_ro(element_addr(sizeof...(ind_types), indarr));
    }
    template <typename... ind_types> inline GenericAccessor operator()(ind_types... inds)
    {
        const SIdx indarr[] = {static_cast<SIdx>(inds)...};
        return this->generic_accessor(element_addr(sizeof...(ind_types), indarr));
    }
    /* Use single uint32 to store tensor format
     ...
     *      Bits 31:20  zero
     *
     *  For other cases: all 0
     *
     *  The value returned by get_tensor_info() is this code, with a 'tensor layout' code
     *  in the upper 4 bits (one of the 'SerOpsInterface::tensMODE_XX' values defined in serialize_oplist.h)
     *  The value serialized as 'tensor_info' has zero in the upper 4 bits.
     *  Classes which cannot be serialized return 0 in the upper 4 bits.
     */
    static inline constexpr uint32_t pack_serialize_data(DType type, uint32_t rank, MemoryClass mclass,
                                                         uint32_t is_padded, uint32_t is_continuous)
    {
        uint32_t tinfo = 0x10;
    ...
    virtual uint32_t get_tensor_info() const; // returns 0
    ...
    std::unique_ptr<Tensor> reallocate_clone(hnnx::Allocator *allocator, bool dup = false) const;
    ...
    virtual void const *read_tile(unsigned flags, void *buffer, size_t b, int h, int w, int d) const;
    ...
    virtual unsigned tile_support_bits() const;
    ...
    static uint32_t content_hash_data(void const *, size_t nbytes, bool is_float) noexcept;
    ...
    static uint32_t build_hash(size_t const *dims, int n, uint32_t previous) noexcept;
...
class FakeTensor : public Tensor {
  public:
    ...
    FakeTensor(const Op *producer_in, hnnx::Deserializer &);

  protected:
    // all will throw exception if called
    virtual void *element_addr(size_t rank, SIdx const coords_in[]) const noexcept override;
    virtual Interface const &interface() const noexcept override;
    virtual void allocate_func(hnnx::Allocator &allocator, unsigned options) override;
    virtual void *raw_data() noexcept override;
    virtual size_t total_storage_elements() const override;
    virtual size_t total_storage_bytes() const override;
    virtual void **clone_util(hnnx::Allocator *allocator, std::unique_ptr<Tensor> *tensp,
                              Tensor::tensor_blockinfo *infop) const override;
    virtual int compare_sametype(const Tensor *rhs) const override;

  public:
    // defined as shown
    virtual const size_t rank() const noexcept override; //->0
    virtual const size_t dim(size_t index) const noexcept override; //->0
    virtual std::pair<size_t const *, size_t> get_dims() const noexcept override; //->{null,0}
    virtual bool set_dims(const size_t dims[]) override; // -> false
    virtual bool set_dims(const Tensor &prototype) override; // ->false
    virtual void enum_memory_blocks(hnnx::MemBlockEnumerator &) const override; // nothing

    virtual DTypeScaleOff get_dtype_intfc() const noexcept override; // { return DTypeScaleOff(None); }
};
...
template <unsigned TRank> class TensorShape : public RankedTensor<TRank> {
  ...
  protected:
    static constexpr NullInterface null_interface{};
  ...
  public:
    ...
    {
        // it would make sense to put Rank in here, but we can't without breaking pickle compat.
        return hnnx::SerOpsInterface::tensMODE_shape << 28u;
    }
...
template <DType DT> class TensorSclrDT : public Tensor {
  ...
  public:
    ...
    virtual uint32_t get_tensor_info() const noexcept override
    {
        // it would make sense to put DType in here, but we can't without breaking pickle compat.
        return hnnx::SerOpsInterface::tensMODE_scalar << 28u;
    }
...
template <typename STYPE, typename TLayout, typename Pad_t> struct layout_mem_contig {
    ...
    layout_mem_contig(Shape_t const *, hnnx::Deserializer &dctx) : bulk_data()
    {
        dctx.allocator->deserialize_blocks(dctx, (void const **)&bulk_data, 1);
        DESERIALIZE_ASSERT_UINT32(dctx, TENSOR_MAGIC);
    }

    // this implements raw_data in the containing tensor
    inline void *raw_data() const noexcept ALWAYS_INLINE { return (void *)bulk_data; }

    // this implements set_raw_data_despite_danger(void *buffer) override final { bulk_data = static_cast<T *>(buffer); }
    inline void set_raw_data_despite_danger(void *buffer) ALWAYS_INLINE
    {
        bulk_data = static_cast<storage_type *>(buffer);
    }

    // this implements element_addr in the containing tensor.
    void *element_addr(Shape_t const *shp, size_t rank, SIdx const coords_in[]) const noexcept ALWAYS_INLINE
    {
        //assert(rank == Rank);
    ...
    inline void **get_block_list_ptr() const ALWAYS_INLINE { return (void **)&bulk_data; }
    static inline size_t get_block_list_len(Shape_t const *shp) ALWAYS_INLINE { return 1; }
    // block size for allocation
    inline static size_t get_elements_per_block(Shape_t const *shp) ALWAYS_INLINE
    {
...
template <typename STYPE, typename TLayout, typename Pad_t> struct layout_mem_indirect {
    ...
    {
        unsigned const nblocks = layout.num_blocks(shp->max_dims);
        blocktab = (storage_type **)indirect_layout_mem::make_blocktab_for_overwrite(nblocks, dctx.graph());
        dctx.allocator->deserialize_blocks(dctx, (void const **)blocktab, nblocks);
        DESERIALIZE_ASSERT_UINT32(dctx, TENSOR_MAGIC);
    }

    // this implements raw_data in the containing tensor
    inline void *raw_data() const noexcept ALWAYS_INLINE { return (void *)blocktab[0]; }
    // this implements set_raw_data_despite_danger(void *buffer) override final { bulk_data = static_cast<T *>(buffer); }
    ...
    // get pointer to block table, and length
    inline void **get_block_list_ptr() const ALWAYS_INLINE { return (void **)blocktab; }
    static inline size_t get_block_list_len(Shape_t const *shp) ALWAYS_INLINE
    {
        return layout.num_blocks(shp->max_dims);
    }
    // block size for allocation
    inline static size_t get_elements_per_block(Shape_t const *) ALWAYS_INLINE { return layout.chunk_total; }
    // find the address of the block pointer containing the specified coords.
    inline storage_type **block_ptr_addr(Shape_t const *shape, std::array<SIdx, Rank> coords) const
    {
...
template <typename Linfo> class LayoutTensor : public RankedTensor<Linfo::Rank> {
  ...
  protected:
    ...
    virtual void *element_addr(size_t rank, const SIdx coords_in[]) const noexcept final override ALWAYS_INLINE
    {
...
template <typename Tinfo> class ConcreteTensor : public LayoutTensor<typename Tinfo::Lconfig> {
  ...
  public:
    ...
    inline const element_type *element_ptr(size_t rank, const SIdx coords[]) const ALWAYS_INLINE
    {
        return (element_type const *)this->element_addr(rank, coords);
    }
    inline element_type *element_ptr(size_t rank, const SIdx coords[]) ALWAYS_INLINE
    {
    ...
    virtual uint32_t get_tensor_info() const noexcept override
    {
        static constexpr uint32_t info = Tensor::pack_serialize_data(traits::dtype, Rank, traits::memclass,
                                                                     BaseLayout::is_padded, !traits::is_indirect);
        return info | (hnnx::SerOpsInterface::tensMODE_general << 28u);
    }
    // allocation and enumeration.
    virtual void allocate_func(hnnx::Allocator &allocator, unsigned options) override final
    {
...
inline std::unique_ptr<T> __attribute__((visibility("hidden")))
make_tensor_template(Op const *op, OutputDef const &odef, Graph &g)
{
