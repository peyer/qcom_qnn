...
class Op : public hnnx::Executable {
  ...
  public:
    ...
    OpId id(const Graph &graph_in) const noexcept;
    ...
    bool is_const() const noexcept; // Data for this op always available, execution and dependence tracking not needed.
     ...
     */
    const char *get_func_name() const noexcept;
  ...
  protected:
    ...
    void enumerate_op_input_blocks(hnnx::MemBlockEnumerator &en, Tensor const *const *inputs_p, unsigned n) const;
    void enumerate_op_output_blocks(hnnx::MemBlockEnumerator &en, hnnx::uptr_Tensor const *outputs_p, unsigned n) const;
    ...
    uint32_t get_serialize_flags(const Graph &, ChkptStoreType st) const;
...
class ConstWrapperOp : public Op {
  ...
  public:
    ...
    ConstWrapperOp(hnnx::Deserializer &dctx);
    ...
    ConstWrapperOp(Graph &graph_in, OpId my_id_in, const OutputDef &def, void const *data_in);

    void clear(Graph *graph_in) override;
    ...
    virtual void serialize(hnnx::SerOpsInterface &sctx) const override;
...
class ShapeWrapperOp : public Op {
  ...
  public:
    ...
    ShapeWrapperOp(hnnx::Deserializer &);
    virtual GraphStatus execute(Graph *g) const noexcept override { return GraphStatus::Success; }
    virtual hnnx::Executable::ItemType compile(Graph &graph_in) const noexcept override
    {
        return hnnx::Executable::null_item();
...
class MetaOpBase : public Op {
  public:
    ...
    virtual std::pair<size_t, size_t> num_inputs_outputs() const override; //{ return {0,0};}
    ...
    virtual uptr_Op clone_meta(Graph &graph_in, OpId new_opid) const; // {return uptr_Op(nullptr);}
};
...
class SpecialPrepOpBase : public MetaOpBase {
  public:
    ...
    virtual bool get_opdef_name(OpId opid, opname_tag_t &result) const; // {return false} in op.cc
