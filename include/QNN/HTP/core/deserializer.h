...
class __attribute__((visibility("default"))) Deserializer : public DeSerError {
  ...
  public:
    ...
    std::vector<uint32_t> extract_const_extent_table(size_t posn_in_words);
    ...
    bool extract_const_extent_data(uint64_t offs_bytes, size_t len, void *dstp, DMA_Manager *dma = nullptr);
...
size_t deserialize_op_idx(Deserializer &dctx);
 ...
 */
uptr_Op deserialize_op(Deserializer &dctx, OpStoreType *op_store_type);
