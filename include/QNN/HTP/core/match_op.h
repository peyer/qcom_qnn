...
class MatchAstNode {
  ...
  public:
    ...
    bool contains_optag(operand_tag_parm_t optag) const;
  ...
  protected:
    ...
    int enumerate_ops(std::map<operand_tag_t, int> &op_tag_to_idx_map, int opcount);
  ...
  public:
    MatchAstNode(char const *, char const *, node_variant, int n_subnodes, MatchAstSubnode *subnodes);
};
...
class MatchBuilder {
  ...
  protected:
    ...
    static typename std::enable_if<have_matchast_uptr<Ts...>::value, hnnx::MatchAst_uptr>::type
            __attribute__((visibility("hidden"))) Op(char const *opname, Ts &&...ts)
    {
        hnnx::MatchAstSubnode subnodes[sizeof...(Ts)] = {std::move(std::forward<Ts>(ts))...};
    ...
    static typename std::enable_if<are_strings<Ts...>::value, hnnx::MatchAst_uptr>::type
            __attribute__((visibility("hidden"))) Op(char const *opname, Ts... ts)
    {
        hnnx::MatchAstSubnode subnodes[sizeof...(Ts)] = {(ts)...};
        return std::make_unique<hnnx::MatchAstNode>(opname, pkg_flag.c_str(), hnnx::MatchAstNode::is_Op, sizeof...(Ts),
                                                    subnodes);
    }
    ...
    template <typename... Ts>
    static typename std::enable_if<are_strings<Ts...>::value, hnnx::MatchAst_uptr>::type
            __attribute__((visibility("hidden"))) Op(char const *opname, hnnx::MatchAst_uptr &&t1, Ts... ts)
    {
    ...
    template <typename... Ts>
    static typename std::enable_if<have_matchast_uptr<Ts...>::value, hnnx::MatchAst_uptr>::type
            __attribute__((visibility("hidden"))) OpVarIn(char const *opname, Ts &&...ts)
    {
        hnnx::MatchAstSubnode subnodes[sizeof...(Ts)] = {std::move(std::forward<Ts>(ts))...};
    ...
    ///
    static hnnx::MatchAst_uptr __attribute__((visibility("hidden")))
    LET(hnnx::operand_tag_parm_t optag, hnnx::MatchAst_uptr &&subnode)
    {
