...
uptr_Op deserialize(Deserializer &dctx, const op_deserializer_fn &info);

template <typename T> struct alloc_func_for_op {
    static void *alloc_func(void *ptr, Deserializer &dctx) { return new (ptr) T(dctx); }
};

__attribute__((visibility("default"))) void deserialize_simple_op_wrapper(void *, Deserializer &dctx,
                                                                          std::unique_ptr<SimpleOpBase> sop_in);
