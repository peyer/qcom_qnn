...
class __attribute__((visibility("default"))) Serializer : public DeSerError, public SerOpsInterface {
  private:
  ...
  public:
    ...
    Serializer(const Graph &graph_in, Allocator *allocator_in, char *p, size_t n)
        : graph_ref(graph_in), bufstart(p), bufend(p + n), bufp(p), bytes_flushed(0), overflowed(false),
          measuring(false), allocator(allocator_in)
    {
    }
    /**
  ...
  protected:
    ...
    unsigned serialize_X_type(type_index_map &tidxmap, std::type_info const &tid, char const *name = nullptr,
                              uint32_t flags = 0);
    ...
    virtual void op_for_tensor_func(Op const *op, unsigned n_out, uptr_Tensor const *out_tens) override;

    // used for all tensor cases
    virtual void tensor_serialize(Tensor const *tens) override;
  ...
  public:
    ...
    void add_op_marker(unsigned op_seqno) override;

    // used to handle 'framework' ops
    virtual OpSerHandle op_special(Op const *op) override;
    ...
    virtual void shape_serialize(ShapeFlags const *basep, unsigned rank) override;
  ...
  protected:
    ...
    void crate_accounting(unsigned obj_size, unsigned obj_align);
    ...
    void do_insert_preload_op(); // actually inserts a preload (just does the accounting, in fact)
    ...
    virtual void spcl_done(OpSerHandle &) override; // called by ~OpSerHandle
    virtual void spcl_add_u32(OpSerHandle &, uint32_t const *p, unsigned n) override;
    virtual void spcl_add_sized_vec(OpSerHandle &, uint32_t const *data, bool extra) override;
    virtual void spcl_fill_nullptr(OpSerHandle &, unsigned n) override;
 ...
 *
 */
class __attribute__((visibility("default"))) NullSerializer : public Serializer {
    union {
        uint32_t for_align;
        char tbuf[64];
    } uu;
 ...
 */
class __attribute__((visibility("default"))) FileSerializer : public Serializer {
  ...
  public:
    ...
    FileSerializer(const Graph &graph_in, Allocator *allocator_in, int file_no, size_t buf_len = MINBUFFER);
    virtual ~FileSerializer();

    // note, finalize can be done *before* rewriting header
    virtual size_t finalize() override;
    ...
    char *prepare_to_modify_header();
    void done_header_modify();
  ...
  private:
    virtual void flush_buffer() override final;
    virtual void serialize_fwrite(const void *p, size_t len, bool align) override final;
};
