...
template <typename Linfo> struct tile_methods_r4flat {
    ...
    static uint8_t const *tile_read(TensType const *tensor, // tensor to read from
                                    unsigned flags,
                                    uint8_t *tbuf, // caller-supplied buffer
                                    size_t b, int h, int w, int d);
    static void tile_write(TensType *tensor, // tensor to store to
                           unsigned flags,
                           uint8_t const *tbuf, // caller-supplied buffer
                           size_t b, int h, int w, int d);
    static constexpr unsigned tile_support_bits()
...
template <unsigned ELBYTES, unsigned RANK = 4> class TileStoreWindow : public TileStoreWindowBase<RANK> {
  public:
    ...
    //
    void write_tile(unsigned flags, void const *tiledata, size_t b, int h, int w, int d);
