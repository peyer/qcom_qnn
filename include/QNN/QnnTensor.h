...
typedef enum {
  ...
  /// Tensor has already been registered with backend. Duplicate info
  ...
  /// A hash collision has occurred with a previously registered tensor's name
 ...
 *                       creation. The _id_ is generated by QNN backend based on _name_. Tensor
 *                       _name_ must be unique in the context within which the tensor is created.
 *                       Any preset value in _id_ will be overwritten by the backend as part of
 *                       this call. Subsequent usage of the tensor must reference this _id_.
 *
 * @return Error code:
 ...
 *         - QNN_TENSOR_ERROR_ALREADY_EXISTS: A tensor with the same name has already been added in
 *           the context
 *         - QNN_TENSOR_ERROR_NAME_HASH_COLLISION: A name hash collision occurred, the
 *           tensor's name must be modified
 ...
 *          multiple graphs.
 *
 * @param[in] graph A handler to the graph which owns this tensor.
 *
 * @param[in,out] tensor Pointer to a user-allocated struct containing information on the tensor
 ...
 *                have the data pointer set to NULL at the time of creation. The _id_ is generated
 *                by QNN backend based on _name_. Tensor _name_ must be unique in the context
 *                within which the tensor is created. Any preset value in _id_ will be overwritten
 *                by the backend as part of this call. Subsequent usage of the tensor must
 *                reference this _id_.
 *
 * @return Error code:
